...
function UpgradeModal({ plan, onClose, onUpgrade }) {
  const opts = [
    { key: 'pro',     label: 'Pro',     price: '$19/mo', limit: '20 req/day, 4 APIs', color: 'from-brand-600 to-indigo-600' },
    { key: 'premium', label: 'Premium', price: '$49/mo', limit: '30 req/day, all APIs', color: 'from-purple-600 to-pink-600' },
  ].filter((o) => o.key !== plan);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-md card-glass rounded-2xl p-6 shadow-2xl animate-scale-in">
        ...
        <div className="space-y-3">
          ...
          <p className="text-center text-xs text-slate-600 pt-1">Secure Checkout via Stripe</p>
        </div>
      </div>
    </div>
  );
...
export default function ApiSelection() {
  ...
  const handleUpgrade = async (targetPlan) => {
    ...
    } catch { showToast('Checkout session failed', 'error'); }
  };
